-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/ravina/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/ravina/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/ravina/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/ravina/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/ravina/outputs' USING PigStorage('\t');
